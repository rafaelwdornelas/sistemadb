...
class Pedidocompra {
  ...
  async imprimir(dados) {
    ...
    if (dados.pedido.DATAAPROVADO == null || dados.pedido.DATAAPROVADO == "") {
      dados.pedido.DATAAPROVADO = "        ";
    }
    doc.autoTable({
      body: [
        [
          {
            content:
              "FORNECEDOR:" +
              `\n[${dados.fornecedor[0].CLI_CPF_CNPJ}] ${dados.fornecedor[0].CLI_NOME}` +
              `\n${dados.fornecedor[0].ENDERECOS[0].END_RUA}, ${dados.fornecedor[0].ENDERECOS[0].END_NUMERO}` +
              `\n${dados.fornecedor[0].ENDERECOS[0].END_BAIRRO} - ${dados.fornecedor[0].ENDERECOS[0].END_MUNICIPIO} - ${dados.fornecedor[0].ENDERECOS[0].END_EST} - ${dados.fornecedor[0].ENDERECOS[0].END_CEP}` +
              `\n(${dados.fornecedor[0].TELEFONES[0].TEL_DDD}) ${dados.fornecedor[0].TELEFONES[0].TEL_NUMERO} ${dados.fornecedor[0].TELEFONES[0].TEL_CONTATO}`,
            styles: {
              halign: "left",
            },
          },
          {
            content:
              `Numero do Pedido: ${dados.pedido.id}` +
              `\nDate: ${dados.pedido.CRIADO_EM.split("T")[0]}` +
              `\nData Aprovado: ${dados.pedido.DATAAPROVADO.replace(
                /(\d{4})(\d{2})(\d{2})/g,
                "$1-$2-$3"
              )}`,
            styles: {
    ...
    doc.autoTable({
      head: [
        ["ID", "Código", "Descrição", "Quant.", "UND", "Vl.Unit.", "Vl.Total"],
      ],
      styles: {
        halign: "left",
        fontSize: 8,
    ...
    doc.autoTable({
      body: [
        [
          "Total Mercadoria: ",
          "R$ " + dados.pedido.VALORMERCADORIAS.toFixed(2),
        ],
        ["Total Taxas: ", "R$ " + dados.pedido.VALORTAXAS.toFixed(2)],
        ["Total: ", "R$ " + dados.pedido.VALORTOTAL.toFixed(2)],
      ],
      theme: "plain",
      columnStyles: {
        0: {
